...
public class BasicOperations {
    ...
    BasicOperations(String str) {
        try {
            ...
            ReadImage ri = new ReadImage(image, image.getWidth(), image.getHeight());
            Color[][] color = ri.pixels; // get array of pixel
            int[] background = ri.determBackground(); // detect font
            Erosion er = new Erosion(color, background); // erosion
            color =  er.erosion();                       // on/off
            FindInBreadth fd = new FindInBreadth(color, background); //finding silhouette
            silhouette = fd.findSilhouette();
