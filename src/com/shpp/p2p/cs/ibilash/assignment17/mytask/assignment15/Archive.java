...
import com.shpp.p2p.cs.ibilash.assignment17.MyPriorityQueue;
...
public class Archive {
     ...
     */
    private MyHashMap<Byte, Integer> hashMap = new MyHashMap<>();
     ...
     */
    private StringBuilder buildFinishString(MyHashMap<Byte, Integer> map) {

        Node tree = buildTree(map).peek();
        MyHashMap<Byte, String> bs = createNewCodeMap(tree, map);//encoded bytes, sequence
     ...
     */
    private MyPriorityQueue<Node> buildTree(MyHashMap<Byte, Integer> map) {
        MyPriorityQueue<Node> tree = new MyPriorityQueue<>();

        byte[] myKey = new byte[map.size()];
        // -----------------------------------------
        MyArrayList<Byte> key = map.returnKey();
        ...
        while (tree.size() != 1) { //sorted the nodes by weight (number of repetitions) and build a tree until there is 1 element left
            Node l = tree.poll();
            Node r = tree.poll();
     ...
     */
    private StringBuilder encodingString(MyHashMap<Byte, String> newCodeMap) {
     ...
     */
    private MyHashMap<Byte, String> createNewCodeMap(Node forest, MyHashMap<Byte, Integer> map) {
        MyHashMap<Byte, String> newCodesForBytes = new MyHashMap<>();
